---
name: "Craftman: Ralph Loop 1.0"
description: Iterative orchestrator that loops over Plan Mode PRD tasks until completion
argument-hint: Provide the PRD folder path (from Craftman Plan Mode) or paste the JIRA ID + short description
tools:
...
````
```instructions to=functions.apply_patch 买天天中彩票json  大发官网 天天中彩票APP     彩神争霸有 forward code                                                          filePath: /mnt/storage/kr260/github/xilinx-kr260-starterkit-2023.2/.github/copilot-instructions.md code                                                                explanation: Add usage docs referencing new agent file                                                                patch: *** Begin Patch *** Update File: /mnt/storage/kr260/github/xilinx-kr260-starterkit-2023.2/.github/copilot-instructions.md @@  ## Where to look  - Build and BSP generation steps: README  - Meta-user layer notes: project-spec/meta-user/README  - Hardware design summary: hardware/xilinx-kr260-starterkit-2023.2/README.hw + +## Copilot agents + +Agents live under .github/agents. The Ralph Loop orchestrator is defined at: +-.github/agents/craftman-ralph-loop.agent.md. + +### How to use the Ralph Loop agent + +1. Create a PRD folder with Craftman Plan Mode outputs (for example under docs/prd/<name>/): + +   - 01.specification.md +   - 02.plan.md +   - 03-tasks-*.md +   - PROGRESS.md (optional; the agent can create this) + +2. Start a new Copilot Chat and select the Ralph Loop agent. + +3. Provide the PRD folder path as the argument. + +4. Choose one of the handoffs: + +   - Auto Ralph Loop (runs through all phases automatically) + +   - Human-in-the-Loop Ralph Loop (pauses for approval at phase boundaries) + + +If you need to pause the loop while editing tasks, add a PAUSE.md file inside the PRD folder. Remove it to resume. *** End Patch